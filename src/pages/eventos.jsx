...
export default function Eventos() {
    ...
    // Atualiza o estado com o evento clicado e mostra o modal
    const abrirModalEntrar = (evento) => {
  ...
  // Dados dos eventos (movido para fora do map para clareza)
            ...
            <button className={styles.btnBusca}>
              ...
              >
                <path
                  ...
                  stroke="white"
                  strokeWidth="2" // Corrigido: strokeWidth em camelCase
                  strokeLinecap="round" // Corrigido: strokeLinecap em camelCase
                  strokeLinejoin="round" // Corrigido: strokeLinejoin em camelCase
                />
                <path
          ...
          <div className={styles.listaEventos}>
            {listaDeEventos.map((evento, index) => (
              <div className={styles.cardEvento} key={index}>
                ...
                {/* O onClick agora chama abrirModalEntrar passando o evento específico */}
                <div onClick={() => abrirModalEntrar(evento)} className={styles.iconeEntrarContainer}>
                  <div className={styles.iconeEntrar}>
                    <BsArrowRight />
                  </div>
                </div>
                {/* O Modal de Entrar não é mais renderizado aqui dentro do map */}
              </div>
            ))}
          </div>
        </div>

        {/* Modal Criar Evento (Renderizado condicionalmente fora do map) */}
        {mostrarModal && <ModalCriar onClose={fecharModal} />}

        {/* Modal Entrar Evento (Renderizado condicionalmente fora do map) */}
        {/* Passa o evento selecionado e um ID único para o modal */}
        {mostrarModalEntrar && eventoSelecionado && (
          <EntrarEventoModal
            evento={eventoSelecionado} // Passa os dados do evento selecionado
            onClose={fecharModalEntrar}
            // Gera um ID único para o input de senha baseado no ID do evento
            inputId={`eventPassword-${eventoSelecionado.id}`}
          />
        )}
      </div>
    </Layout>
